...
_VIDEO_ANALYSIS_DIR = Path(
    os.getenv(
        "VIDEO_ANALYSIS_DIR",
        str(Path(__file__).resolve().parents[2] / "artifacts" / "video_analysis_jobs"),
    )
)
...
@app.post("/video-analysis/uploads", status_code=201)
async def create_video_analysis_upload(
    ...
    job = models.DBVideoAnalysisJob(
        ...
        status="draft",
        expires_at=datetime.utcnow() + timedelta(seconds=max(1, _VIDEO_ANALYSIS_RETENTION_SECONDS)),
    )
    db.add(job)
    db.commit()
    db.refresh(job)
...
@app.get("/video-analysis/jobs/{job_id}/preview")
...
@app.post("/video-analysis/jobs/{job_id}/run", status_code=202)
def run_video_analysis_job(job_id: str, request: VideoAnalysisRunRequest, db: Session = Depends(get_db)):
    ...
    for filename, _media_type in _VIDEO_ANALYSIS_ARTIFACTS.values():
...
@app.get("/video-analysis/jobs/{job_id}/artifacts/{artifact_name}")
...
@app.delete("/video-analysis/jobs/{job_id}")
...
def _get_video_analysis_job(db: Session, job_id: str) -> models.DBVideoAnalysisJob:
    job = db.query(models.DBVideoAnalysisJob).filter(
...
def _recover_video_analysis_jobs(db: Session):
    interrupted_jobs = db.query(models.DBVideoAnalysisJob).filter(
