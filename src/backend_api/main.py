...
_RTI_SNAPSHOT_PATH = dashboard_dir / "rti_data.json"


@app.get("/rti/snapshot")
def get_rti_snapshot():
    """Mobility intelligence + RTC surveillance aggregates for the dashboard."""
    if not _RTI_SNAPSHOT_PATH.is_file():
        raise HTTPException(status_code=404, detail="RTI snapshot not found")
    with open(_RTI_SNAPSHOT_PATH, encoding="utf-8") as f:
        return json.load(f)
